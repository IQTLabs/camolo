...
class MaxProbExtractor(nn.Module):
    ...
    def forward(self, YOLOoutput, verbose=False):
        ...
        # AVE Edit below (4 class VisDrone gives nans for output[:, 4, :], so clamp it?)
        #z = torch.nan_to_num(output[:, 4, :], nan=-1e6, posinf=1e6, neginf=-1e6)
        #output_objectness = torch.sigmoid(z)
        if verbose:
            print("load_data.py - MaxProbExtractor.forward(): output.shape", output.shape)
            print("load_data.py - MaxProbExtractor.forward(): output[:, 0, :]", output[:, 0, :])
            print("load_data.py - MaxProbExtractor.forward(): output[:, 5, :]", output[:, 5, :])
            print("load_data.py - MaxProbExtractor.forward(): output[:, 4, :]", output[:, 4, :])
        #print("load_data.py - MaxProbExtractor.forward(): z", z)
        output_objectness = torch.sigmoid(output[:, 4, :])  # [batch, 1805] # original
...
class PatchTransformer(nn.Module):
    ...
    def __init__(self, target_y_offset=0.0, target_size_frac=0.35):
        super(PatchTransformer, self).__init__()        
        self.target_y_offset = target_y_offset # # Originally -0.05
    ...
    def forward(self, adv_patch, lab_batch, img_size, do_rotate=True, rand_loc=True):
        ...
        # AVE EDIT: Set target patch size
        ...
        # AVE EDIT: Not sure why we are subtracting 0.05 from target_y, but this may result in the observed 
        # vertical patch offset (patch is placed above the car for COWC)
        target_y = target_y + self.target_y_offset
        # Original Below
...
class PatchApplier(nn.Module):
    ...
    def __init__(self, patch_alpha=1):
    ...
    def forward(self, img_batch, adv_batch):
        ...
        for adv in advs:
            ...
            # # Add patch values to image values? 
            # (This doesn't actually work properly, since we can't add to white)
            # if self.sum_patch_and_image:
            #     img_batch = img_batch.add(adv)
...
class CamoDataset(Dataset):
    ...
    def pad_and_scale_simp(self, img, lab, verbose=False):
        """
    ...
    def pad_and_scale(self, img, lab, verbose=False):
        """
        ...
        w,h = img.size
        if verbose:
            print("load_data.py - pad_and_scale(): img.size:", img.size)
        if w==h:
            padded_img = img
        else:
            dim_to_pad = 1 if w<h else 2
        ...
        padded_img = resize(padded_img)     #choose here
        if verbose:
             print("load_data.py - pad_and_scale(): padded_img.size:", padded_img.size)
        return padded_img, lab
...
if __name__ == '__main__':
    ...
    prob_extractor = MaxProbExtractor(0, 1).cuda()
