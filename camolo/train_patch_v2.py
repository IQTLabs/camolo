...
Include a loss term that penalizes an easily detectable patch.

To run:
python train_patch.py [patch_detect_model] [patch_model] 
e.g.:    python train_patch_v2.py visdrone_patch_detect_v0 visdrone_v1_4cat_obj_only_tiny_gray_v1
...
class PatchTrainer(object):
    def __init__(self, detect_mode, patch_mode):
        
        self.config = patch_config.patch_configs[patch_mode]()
        ...
        self.writer = self.init_tensorboard(patch_mode)

        # detect mode
        self.config_detect = patch_config.patch_configs[detect_mode]()
        self.darknet_model_detect = Darknet(self.config_detect.cfgfile)
        self.darknet_model_detect.load_weights(self.config_detect.weightfile)        
        self.darknet_model_detect = self.darknet_model_detect.eval().cuda() # TODO: Why eval?
        self.prob_extractor_detect = MaxProbExtractor(self.config_detect.cls_id, 
                                                      self.config_detect.num_cls, 
                                                      self.config_detect).cuda()
    ...
    def train(self):
        ...
        print("Output directory:", patch_dir)
        ...
        for epoch in range(n_epochs):
            ...
            for i_batch, (img_batch, lab_batch) in tqdm(enumerate(train_loader), desc=f'Running epoch {epoch}',
                ...
                with autograd.detect_anomaly():
                    ...
                    # get patch_detect loss
                    output_detect = self.darknet_model_detect(p_img_batch)
                    max_prob_detect = self.prob_extractor_detect(output_detect)                    
                    det_loss_detect0 = torch.mean(max_prob_detect)
                    det_loss_detect = det_loss_detect0 * self.config.patch_detect_mult
                    
                    # get total loss
                    loss = det_loss + det_loss_detect + nps_loss + torch.max(tv_loss, torch.tensor(0.1).cuda())

                    ep_det_loss += det_loss.detach().cpu().numpy()
                    ep_det_detect_loss += det_loss_detect.detach().cpu().numpy()
                    ...
                    if i_batch%5 == 0:
                        ...
                        self.writer.add_scalar('loss/det_loss_detect', det_loss_detect.detach().cpu().numpy(), iteration)
            ...
            ep_det_detect_loss = ep_det_detect_loss/len(train_loader)
            ...
            if True:
                ...
                logging.info('  DET_Det LOSS: {}'.format(ep_det_detect_loss))
...
def main():
    if len(sys.argv) != 3:
        print('You need to supply a patch_detect_model (argv1) and patch_model (argv2).')
